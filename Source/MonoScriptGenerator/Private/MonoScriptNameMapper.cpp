...
//FIXME: build a general purpose name remapping system
FString MapObjectName(FString &ObjName, const FString& StripPrefix)
{
	//ESourceEffectFilter collides with USourceEffectFilter
	if (ObjName == TEXT("ESourceEffectFilter"))
	{
		return TEXT("SourceEffectFilterType");
	}

	ObjName.RemoveFromStart(StripPrefix);

	return ObjName;
}

FString MonoScriptNameMapper::GetQualifiedName(const UClass& Class) const
{
	const FMonoBindingsModule& BindingsModule = MonoModuleFinder.FindModule(Class);

	return FString::Printf(TEXT("%s.%s"), *BindingsModule.GetNamespace(), *MapClassName(&Class));
}

FString MonoScriptNameMapper::GetQualifiedName(const UScriptStruct& Struct) const
{
	const FMonoBindingsModule& BindingsModule = MonoModuleFinder.FindModule(Struct);

	return FString::Printf(TEXT("%s.%s"), *BindingsModule.GetNamespace(), *MapStructName(&Struct));
}

FString MonoScriptNameMapper::GetQualifiedName(const UEnum& Enum) const
{
	const FMonoBindingsModule& BindingsModule = MonoModuleFinder.FindModule(Enum);

	return FString::Printf(TEXT("%s.%s"), *BindingsModule.GetNamespace(), *MapEnumName(&Enum));
}
...
FString MonoScriptNameMapper::ScriptifyName(const FString& InName, const EScriptNameKind InNameKind) const
{
	check(InName.Len());

	FString MappedName = ScriptNameMapper::ScriptifyName(InName, InNameKind);
